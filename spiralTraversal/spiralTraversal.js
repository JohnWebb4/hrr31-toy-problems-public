...
const spiralTraversal = function(matrix) {
  ...
  // Recurse
  let firstRow = matrix.slice(0, 1).reduce((flatenned, row) => ( flatenned.concat(row) ), []);
  let remainSpiral = spiralTraversal(rotateArrayCounterClockwise90Deg(matrix.slice(1)));

  return firstRow.concat(remainSpiral);
};

const rotateArrayCounterClockwise90Deg = function(matrix) {
  return matrix.reduce((rotMatrix, row) => {
