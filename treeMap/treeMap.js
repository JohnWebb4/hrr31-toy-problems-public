 ...
 *
 * Start at root node
 * Call map on root node and store as new tree root node
 * Iterate through children
 ** Recursively call on children
 ** Add resulting mapped child to node
*/
...
Tree.prototype.map = function map(callback) {
  // return a new tree with the same structure as `this`, with values generated by the callback
  // Base Case
  const mappedTree = new Tree(callback(this.value));

  // Recursive Case
  this.children.forEach((child) => {
    mappedTree.addChild(child.map(callback));
  });
...
Tree.prototype.mapInPlace = function mapInPlace(callback) {
